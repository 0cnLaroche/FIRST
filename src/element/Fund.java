...
public class Fund extends Pane {
	...
	public Fund(model.Fund fund) {
		
		this.vb = new VBox();
		this.details = new HBox();
		this.definition = new Label(fund.getDefinition());

		this.id = new Label(fund.getId());
		this.name = new Label(fund.getNameEN());
		...
		if (fund.getDefinition().length() > 0) {
